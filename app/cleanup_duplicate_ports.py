...
問題:
- YAMLインポートとフォールバック生成の両方が実行され、重複ポートが作成された
- 例: DispenseLiquid96Wellsに`out1`(正しい)と`output_2`(重複)の両方が存在
- `output_2`が接続(port_connections)で使用されている

解決策:
1. `output_2`を使用する接続を`out1`に移行
2. 重複した`output_2`ポートを削除
"""
...
# パスを追加
sys.path.append(str(Path(__file__).parent.parent / "labcode-test-environment" / "labcode-log-server" / "app"))
...
# データベース接続
DB_PATH = Path(__file__).parent.parent / "labcode-test-environment" / "labcode-log-server" / "data" / "sql_app.db"
